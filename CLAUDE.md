...
- **Khmer combining marks are letters here.** Normalization keeps `\p{L}\p{N}\p{M}` — dropping `\p{M}` shreds every Khmer title into a consonant skeleton. Similarity is the stronger of a token measure and a code-point edit distance, because a Khmer title is legitimately one token.
