...
- **Every storage path segment is built by `lib/storage/folder-name.ts`, and it is the only place that knows the rule.** Zima validates each `/`-separated segment of `x-folder` with `/^[a-zA-Z0-9_\- ក-៿]{1,80}$/` (its `lib/safeFiles.js` → `isValidFolderPath`) and answers anything else with a bare `400 {"error":"Invalid target folder"}` — before a byte is written. Three consequences: **80 is a per-segment CHARACTER cap**, not bytes and not a whole-path budget (Khmer is BMP, so it counts the same as ASCII); **non-ASCII cannot travel in that header at all**, because `zimaUpload()` percent-encodes it and `%` is not in the charset, which is why storage segments stay ASCII and the uid carries uniqueness; and **`.` is not in the folder charset** (file names are separate). `buildStorageFolderName()` reserves the uid first and truncates the slug to a 64-char budget on a word boundary; `bookFolder`/`postFolder`/`thesisFolder`/`publicationFolder` in `lib/book-utils.ts` are the only callers you should need. Both upload routes and `validateFolder()` re-check with `describeStorageKeyError()` so an operator gets a sentence, not `Invalid target folder`. Every academic title with a subtitle overran this before the clamp existed and simply could not be uploaded. Pinned by `lib/storage/folder-name.test.ts`, which asserts against a copy of the server's own regex.
- **Zima meters uploads per REQUEST, not per file, on two independent counters**: `POST /api/upload` (`upload:<ip>`, `RL_UPLOAD_PER_HOUR`=60, 1 file) and `POST /api/v1/files` (`v1-upload:<ip>`, `storageUploadPerHour`=120, up to 10 files, `storage:write` scope). Both are keyed by client IP and every upload reaches Zima from THIS server, so the whole app shares each bucket. The bulk importer sends a book's PDF and cover in ONE v1 request — 1 of 120 instead of 2 of 60, **120 books/hour instead of 30**, with no limit raised or removed. Raising `RL_UPLOAD_PER_HOUR` is the escape hatch, not the fix: the quota is a RATE, so concurrency changes nothing. v1 takes one `folder` per request, so it is one request per book, not several books per request. A v1 failure falls back to the legacy path and the response's `via` field makes Step 3 say which path ran — a fallback and a fast run are otherwise indistinguishable. Partial success is asymmetric on purpose: a failed PDF trashes everything the batch stored, a failed cover still saves the book but shows a warning. The quota is charged BEFORE folder validation, so a rejected row still spends it; the duplicate check sits ahead of the storage call so a re-run costs zero quota. Progress persists in `book_import_runs` (`0129`) so a long pause survives a refresh; the FILES do not persist, so resuming re-selects the folders. Full picture: `docs/BULK-IMPORT.md`.
- **Large files go through a SESSION, not a request.** `/api/admin/upload/chunk` is a four-verb protocol (`init`, `chunk`, `finalize`, plus `GET` for status and `DELETE` for cancel) over a durable `upload_sessions` row (`0132`) and a durable staging directory (`UPLOAD_STAGING_DIR`, a Docker volume — **never `os.tmpdir()`, which is a tmpfs in the production container and is erased by every deploy**). Finalization is never implicit: the old route finalized on the last chunk, so assembly, hashing, a VirusTotal round-trip, a duplicate query and the whole upload to Zima happened inside the request carrying the final 5 MB, with a progress bar that had already reached 100%. Four rules carry the fix: **finalize is claimed with a compare-and-set** on the session row, so a client timeout cannot race its own still-running finalize into a second storage object (that race, plus the tmpfs wipe, plus `rm -rf` on every error path, is where "Missing chunk 0 during final assembly" came from — three separate producers); **a finished finalize replays its result** rather than redoing the work; **a transient failure keeps the staged bytes** and hands the session back to `UPLOADING`, so a retry costs no re-upload, while only a terminal refusal (duplicate/malware/wrong type/oversize) discards them; and **nothing is buffered** — the hash is streamed and `zimaUploadStream()` writes the multipart envelope around a `Readable` with an exact `Content-Length`, so peak memory is a 1 MB window at any file size instead of ~6× the file (measured: 148 MB RSS for a 25 MB book, before). `lib/uploads/state.ts` is pure and owns the transition table, the error-code enum and `MAX_UPLOAD_BYTES` — which is **100 MiB minus one byte**, because Zima refuses exactly 104,857,600 and the app used to allow precisely that size. Session ownership is enforced per verb (`requireOwnedSession`): staff authorization says you may upload, not whose staged bytes are yours. Full picture: `docs/CHUNKED-UPLOAD.md`.
- **An upload is not done until the row exists.** `saveBookRecord({ uploadId })` is the second half of a two-phase commit: it claims the session `STORED → SAVING_DB`, inserts, then records `COMPLETED` with the book id — so a repeat of the same submit replays that book instead of inserting a second, and a failed insert returns the session to `STORED` (the FILE is still good; failing it would strand a 95 MB upload over a bad category name). The browser no longer deletes a stored PDF when a save errors: a save request can fail from the browser's point of view while the insert succeeded, and the file it deleted was then the one a live row pointed at. `/api/cron/upload-reconcile` (hourly) releases sessions stuck mid-transition, **adopts** a file some row turns out to reference, flags the rest as orphans, and reclaims staging disk — it never deletes a database row, and never deletes a storage object without asking the database first, with a failed lookup counting as "referenced".
- **`books.storage_folder` (`0128`) records where a book's files went — never recompute it from the title.** The uid in a folder name is random and the slug is truncated, so reconstruction is impossible, and a title edit changes the input anyway. The edit form's old fallback minted a *new* folder on every PDF replacement for any book without a cover.
...
- **Indexing runs in the background, so its outcome is a DATABASE ROW, not a log line** (`resource_index_state`, `0133`). `indexPdfPagesSafe()` is non-throwing by contract, and for five weeks that meant a total production failure — `pdf.worker.mjs` was not traced into `.next/standalone`, so `getDocument()` threw on its first statement for all 120 books — rendered exactly like a library of scans: `book_pages` 0 rows, no phrase search inside any book, no page an AI answer could cite, and nothing in the app able to say so. Four statuses, and the splits are the point: `no_text_layer` is a permanent fact about the document, `unfetchable` is transient, `failed` is **always** a bug on our side (`outcomeFromError()` maps every throw there — never to `no_text_layer`), and `never_attempted` stays its own bucket in `public_resource_index_health` rather than being folded into a "not indexed" total. The pure mapping lives in `lib/indexing/state.ts`; the admin Data Quality panel renders full-text coverage beside embedding coverage because a collection can score perfectly on one and hold nothing on the other. `outputFileTracingIncludes` in `next.config.ts` ships the worker and is load-bearing for search and RAG, not a build detail. pdf.js hides a SECOND dependency the same way — it reaches for `@napi-rs/canvas` (an optional dep) through `createRequire` to get a `DOMMatrix`, which Node lacks and the traced bundle therefore lacks too, so `pdf.mjs` throws `DOMMatrix is not defined` while evaluating its own module body. `lib/polyfills/dom-matrix.ts` supplies a narrow 2D one, mounted before the import; a 3D operation on it throws rather than answering wrongly. Full picture + backfill runbook: `docs/DISCOVERY-DATA-QUALITY.md`.
...
- **Cron routes** under `app/api/cron/` are `Bearer $CRON_SECRET`-authenticated and platform-neutral — the self-hosted container schedules nothing itself, so `.github/workflows/cron.yml` is the scheduler of record (publish sweep every 15 min, cleanup daily at 20:00 UTC; its `CRON_SECRET` repo secret must match the box's `.env`). They are: `security-scan` runs the detection + incident + notification pass every 5 minutes (see Security Monitoring above); `upload-reconcile` runs hourly over `upload_sessions` (see Large files go through a SESSION above); `publish-scheduled` flips posts/theses/books from `scheduled` → `published` once `scheduled_at` passes (DB triggers `0073`/`0075`/`0086` then cascade `is_published`/`published_at`) and runs the Announcement Center sweep (`0100`); `cleanup` does retention deletes.
...
### Book Ingestion & Duplicate Prevention

`/admin/books/upload` is a quality gate, not a form. Full picture: `docs/BOOK-INGESTION.md`.

- **Duplicate identity is deterministic — a model never decides it.** Gemini drafts metadata (`app/actions/ai-extraction.ts`) and the form labels it as a draft; every input to the duplicate decision is a database fact or a pure function of one. The rules live in `lib/books/duplicate-detection/*` (pure, browser-safe, unit-tested offline); only `service.ts` touches a database, and it scores nothing.
- **One definition of "the same string", for three doors.** `lib/admin/duplicates.ts` (the review queue) re-exports `normalizeTitle`/`normalizeIsbn` from there rather than keeping its own, and the bulk importer's old exact `title|author` key is gone. Three copies meant an ISBN collision sailed through a CSV import while a second edition was refused.
- **Only an identifier may BLOCK.** A shared content hash (100) or a shared ISBN (97) refuses the save; title/author/year evidence is capped at 94 (`NON_IDENTIFIER_CEILING`) because two editions of one textbook agree on all three. Bands: 95+ blocks, 80–94 strong warning, 60–79 review, under 60 unreported. Different valid ISBNs and different declared editions (`2nd Edition`, `បោះពុម្ពលើកទី២`) DEMOTE a match; `isSeriesVariant()` drops pairs whose titles differ only in their digits, because this collection is mostly grade-numbered textbooks.
- **A hash is an invariant; an ISBN is a policy.** `book_files.content_hash` keeps its unique index (0060) and is never overridable. An ISBN collision is overridable exactly once, only when the acknowledged book id still matches what the server finds, and the result is routed to `/admin/review` rather than published. Both the block and the override write an `admin_actions` row. Do NOT add a unique index on ISBN — it would make the override impossible.
- **`normalizeIsbn()` canonicalises to ISBN-13 and is lenient about the check digit** (two rows sharing a mistyped ISBN are still one book twice). Validity is a separate answer from `validateIsbn()`, shown as a field warning; an invalid ISBN never blocks a save.
- **Khmer combining marks are letters here.** Normalization keeps `\p{L}\p{N}\p{M}` — dropping `\p{M}` shreds every Khmer title into a consonant skeleton.
- **A shared title FRAME is not shared identity, and the character measure is not a general-purpose one.** `titleSimilarity()` took `Math.max` of a token measure and whole-string edit distance; edit distance reads a 3-character difference inside a 50-character title as 94% alike, so 15 Khmer teacher's guides named `សៀវភៅណែនាំគ្រូបង្រៀន {SUBJECT} ថ្នាក់ទី{GRADE} (STEPSAM3)` were each refused as a copy of the last — the token measure had said 66 and lost the `max`. Edit distance now runs only where it is the ONLY informed measure (a single-token, space-less Khmer title, the case it was added for); once both titles have words, `fuzzyTokenRatio()` decides, crediting an unmatched token a fraction of a match equal to its character similarity so a misspelling still scores ~0.9 while a different word scores nothing. `isDistinguishingVariant()` covers the space-less case: strip the shared opening and ending, and ≥3 mutually unrecognisable code points on both sides means different books. `titleWithoutEdition()` makes two editions of one work agree on their base rather than relying on distance to rate them alike. None of it can touch identifier evidence — hash and ISBN have already returned. Pinned by `lib/books/duplicate-detection/stepsam3.test.ts`.
- **The bulk importer's pre-flight verdict is always overridable, per row.** It scores metadata against a pool with no content hashes, so it can be wrong, and a false "already in the library" with no way through is worse than the duplicate it prevents. Overriding flips the row back to `pending` and writes a `book.import_duplicate_override` audit row; the authoritative refusal is still `assertNotDuplicate` at save time with the real hash, behind the unique index on `book_files.content_hash`, and THAT one is never overridable.
- **Migration 0130 does RECALL, not precision.** `find_book_duplicate_candidates()` returns a bounded, separately-capped candidate set per branch (hash / ISBN digits / exact folded title / trigram / prefix / author) and the TypeScript scorer decides. Reproducing `normalizeTitle()` in SQL was rejected: the two would drift and the drift would read as "no duplicate found". Never add `SET pg_trgm.word_similarity_threshold` to a function — the GUC is unregistered until the extension's library loads, and `CREATE FUNCTION` then fails with `permission denied to set parameter`. Both 0130 functions are service-role only; PostgREST would otherwise expose unpublished titles to `anon`.
...
| `lib/pdf-worker-tracing.test.ts` | pdf.js's worker is shipped to the standalone server — it is named by a runtime global, so nothing else can see the dependency (this is the file whose absence emptied the whole full-text index in production) |
| `lib/indexing/state.test.ts` | a thrown indexing error is recorded as `failed`, never as a legitimate skip; the four index statuses stay distinguishable |
...
| `lib/uploads/state.test.ts` | the upload state machine's legal edges — no path reaches COMPLETED without passing through STORED, no terminal state reopens, and only `sending` may draw a determinate bar |
