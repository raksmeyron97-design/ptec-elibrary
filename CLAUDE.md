...
- **Launch surface** — `lib/pwa/launch.ts` owns `PWA_INK` (`#060B1A`, the `.hero-ink` colour the app actually paints first in *both* themes). Four systems must agree on it or the launch flashes: `app/manifest.ts` (`background_color` + `theme_color`), `rootViewport.themeColor`, the `apple-touch-startup-image` links in `RootShell`, and `components/pwa/PTECBootScreen.tsx`. `lib/pwa/launch.test.ts` pins all four. The manifest used to say `#ffffff` while the app painted near-black, which is what made every cold launch flash white→dark.
