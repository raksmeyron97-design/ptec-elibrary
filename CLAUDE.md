...
- **Zima allows 60 uploads per HOUR** (`RL_UPLOAD_PER_HOUR` on the storage box; `ZIMA_UPLOADS_PER_HOUR` in `lib/zima.ts` records it), counted per FILE, in a FIXED window, keyed by client IP — and every upload reaches Zima from this server, so the whole app shares one bucket. The quota is charged *before* the folder is validated, so a rejected row still spends it. An 86-row import is ~172 files ≈ 2.9 hours: **concurrency is not the fix, because the quota is a rate, not a concurrency limit** — raising `RL_UPLOAD_PER_HOUR` on the box is. The importer survives it instead of failing: `lib/admin/import-queue.ts` holds one gate shared by all workers (the first 429 stops everyone), honours `retryAfterSeconds` clamped to 70 min, does NOT charge a quota wait to the transient-retry budget, and both upload routes now relay `ZimaUploadError` with its real status instead of flattening everything to 500 — that flattening is what made a rate limit look like a broken file and failed 63 rows in seconds. Progress persists in `book_import_runs` (`0129`) so a long pause survives a refresh; the FILES do not persist, so resuming re-selects the folders. Full picture: `docs/BULK-IMPORT.md`.
