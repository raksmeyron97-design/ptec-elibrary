  ...
  return (
    <Modal labelledBy={headingId} onClose={onCancel}>
      <div className="mb-4 flex items-center justify-between">
        <h2 id={headingId} className="text-lg font-bold text-text-heading">Assign role</h2>
        <CloseButton onClose={onCancel} />
      </div>
      <p className="mb-4 text-sm text-text-muted">
        Choose the system permission for <span className="font-semibold text-text-body">{userLabel(user)}</span>.
      </p>
            ...
            >
              ...
              <span>
                <span className="flex items-center gap-2 text-sm font-bold text-text-heading">
                  {ROLE_META[r].label}
                  {disabled && <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-text-muted">super admin only</span>}
                </span>
                <span className="mt-0.5 block text-xs text-text-muted">{ROLE_META[r].description}</span>
              </span>
            </button>
          );
        })}
      ...
      <div className="mt-6 flex items-center justify-end gap-3">
        <button type="button" onClick={onCancel} disabled={busy} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-text-body hover:bg-paper disabled:opacity-50">Cancel</button>
        <button
          type="button"
