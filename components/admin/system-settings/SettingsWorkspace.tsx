    ...
    */
    ...
    >
      ...
      {data.storageReady && !data.canWrite && (
        <div className="mb-5 rounded-2xl border border-info-line bg-info-soft p-4 text-sm text-info-text" role="status">
              ...
              <div className="sticky bottom-0 z-10 -mx-1 mt-6 rounded-2xl border border-divider bg-bg-surface/95 p-3.5 shadow-lg backdrop-blur">
                <div className="flex flex-wrap items-center gap-3">
                  <div className="min-w-0 flex-1 text-xs text-text-muted">
                    {isDirty(section) ? (
                      <span className="font-semibold text-warning-text">● Unsaved changes</span>
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    {(data.sections[section].draft || isDirty(section)) && (
                      <button
                        ...
                        className="rounded-xl border border-divider px-3.5 py-2 text-sm font-semibold text-text-body transition-colors hover:bg-paper disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        Discard draft
                      </button>
                    )}
      ...
      {publishConfirm && (
        ...
        >
          <div className="w-full max-w-lg rounded-2xl bg-bg-surface p-6 shadow-2xl">
            ...
            {changedVsPublished.length > 0 && (
              <div className="mt-3 max-h-32 overflow-y-auto rounded-xl bg-paper p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-text-muted/70">
                  Changing {changedVsPublished.length} field{changedVsPublished.length === 1 ? "" : "s"}
                </p>
                <p className="mt-1 break-words font-mono text-[11px] text-text-body">
                  {changedVsPublished.join(", ")}
                </p>
              </div>
            )}
            <label htmlFor="publish-comment" className="mt-4 block text-[13px] font-semibold text-text-body">
              Comment <span className="font-normal text-text-muted/70">(optional, shown in history)</span>
            </label>
            <input
              id="publish-comment"
              type="text"
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                className="rounded-xl border border-divider px-4 py-2 text-sm font-semibold text-text-body hover:bg-paper"
              >
                Cancel
              </button>
              <button
