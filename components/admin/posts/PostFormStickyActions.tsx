...
const AUTOSAVE_DISPLAY: Record<Exclude<AutosaveStatus, "idle">, { label: string; className: string; icon: React.ReactNode }> = {
  ...
  saved: { label: "Draft saved", className: "text-emerald-600", icon: <Check className="h-3 w-3" /> },
  ...
  return (
    <div className="sticky top-[64px] z-20 flex flex-wrap items-center gap-2.5 rounded-xl border border-divider bg-bg-surface/95 p-3 shadow-md backdrop-blur">
      ...
      {showDraftShortcut && (
        ...
        >
          {isEdit && wasPublished ? "Unpublish" : "Save as Draft"}
        </button>
      )}
