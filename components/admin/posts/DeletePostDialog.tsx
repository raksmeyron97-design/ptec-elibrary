    ...
    >
      ...
      >
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            <h2 id={headingId} className="text-lg font-bold text-text-heading">
              {isBulk ? `Delete ${count} posts?` : "Delete this post?"}
            </h2>
            <p className="mt-1.5 text-sm text-text-body">
              {isBulk ? (
                <>You are about to delete <strong>{count} posts</strong>. This action cannot be undone.</>
              ) : (
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {busy ? "Deleting…" : "Delete post" + (isBulk ? "s" : "")}
          </button>
        </div>
      </div>
    </div>
