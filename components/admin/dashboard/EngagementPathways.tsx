    ...
    return (
      <div key={key} className="flex min-w-0 items-center gap-2.5 rounded-xl bg-paper/55 px-2.5 py-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1 text-xs font-semibold text-text-body">
          ...
          <p className="mt-0.5 text-xs font-bold uppercase tracking-wide text-text-muted">
            {t(`attribution.${ATTRIBUTION[key]}`)}
          </p>
        </div>
        <div className="shrink-0 text-end">
          <p className="text-xl font-bold leading-tight tabular-nums text-text-heading">
            {collecting
              ? <span className="text-xs font-semibold text-info-text">{t("collecting")}</span>
          ...
          {!collecting && rate.pct !== null && !rate.comparable && (
            <p className="max-w-[128px] text-xs leading-3 text-text-muted">{t("notComparable")}</p>
          )}
        </div>
      </div>
    );
        ...
        <article className="rounded-2xl border border-accent/30 border-s-[3px] border-s-[var(--ptec-accent-line)] bg-accent/[0.05] p-3 md:col-span-2 xl:col-span-1">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs leading-4 text-text-muted">{t("conversionShortDef")}</p>
            </div>
            <div className="shrink-0 text-end">
              <p className="text-xl font-bold leading-none tabular-nums text-text-heading">
                {conversion.insufficient || conversion.valuePct === null
                  ? <span className="text-xs font-semibold text-text-muted">{t("conversionInsufficient")}</span>
                  : `${conversion.valuePct}%`}
              </p>
              {compare && !conversion.insufficient && conversion.prevPct !== null && (
                <p className="mt-1 text-xs tabular-nums text-text-muted">{t("prevPct", { pct: conversion.prevPct })}</p>
              )}
            </div>
          </div>
        </article>
        ...
        <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 sm:grid-cols-5 xl:grid-cols-2">
          {(
            ...
            <div key={key} className="min-w-0">
              <dt className="dash-truncate text-xs font-medium text-text-muted">{t(`volumes.${key}`)}</dt>
              <dd className="text-sm font-bold tabular-nums text-text-heading">
                {value === null ? t("collectingShort") : nf.format(value)}
                {compare && previous !== null && previous > 0 && value !== null && (
                  <span className="ms-1 text-xs font-normal text-text-muted">{t("prev", { value: nf.format(previous) })}</span>
                )}
              </dd>
            </div>
          ))}
