...
export default async function ContentView({
  ...
  const trendIcon = (delta: number) =>
    delta > 0 ? (
      <span className="inline-flex items-center gap-0.5 text-[var(--ptec-success)]">
        <TrendingUp className="h-3 w-3" aria-hidden="true" />
        <span className="tabular-nums">+{nf.format(delta)}</span>
      </span>
    ) : delta < 0 ? (
      <span className="inline-flex items-center gap-0.5 text-[var(--ptec-danger)]">
  ...
  /* One neutral chip for every content type. Four pastel hues (indigo /
     violet / cyan / amber) put a fifth colour family on a page that already
     carries the four validated series hues and four status colours — and they
     encoded nothing the label beside them did not already say. The type is
     read, not decoded. */
  const typeBadge = (type: string) => (
    <span className="dash-status--neutral dash-chip whitespace-nowrap text-xs font-semibold">
      {t(`types.${type}`)}
    </span>
  );
  ...
  const pctChip = (pctVal: number, missing: string[]) => {
    const cls =
      pctVal >= 80 ? "dash-status--ok" : pctVal >= 50 ? "dash-status--warn" : "dash-status--crit";
    return (
      <span className={`${cls} dash-chip text-xs font-bold tabular-nums`} title={missingLabel(missing)}>
        {pctVal}%
      </span>
    );
  };
    ...
    <div className="space-y-8">
        ...
        <nav aria-label={t("presetsLabel")} className="flex items-center gap-1 overflow-x-auto">
          {PRIMARY_CONTENT_PRESETS.map((p) => (
            <Link
              ...
              className={`inline-block whitespace-nowrap rounded-lg px-2.5 py-1.5 text-xs font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand ${
                p === preset
        ...
        <form action="/admin" method="get" className="ms-auto flex items-center gap-1.5" role="search">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="h-8 w-44 rounded-lg border border-divider bg-bg-surface ps-7 pe-2 text-xs text-text-body placeholder:text-text-muted focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-brand sm:w-56"
            />
          </div>
        </form>
      </div>
      ...
      <section aria-labelledby="content-table-heading" className="dash-card overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-2 px-4 pt-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <h3 id="content-table-heading" className="text-sm font-bold text-text-heading">
              {t("tableTitle")}
            </h3>
          </div>
        ...
        {data.rows.length === 0 ? (
          <p className="px-4 py-10 text-center text-sm text-text-muted">{t("emptyPreset")}</p>
        ) : (
                    ...
                    <td className="max-w-[300px] px-4 py-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="dash-truncate-head font-semibold text-text-heading" dir="auto" title={row.title}>
                            {row.title}
                          </p>
                          <p className="dash-truncate text-xs text-text-muted">
                    ...
                    <td className="whitespace-nowrap px-2 py-2">
                      {row.language ? (
                        <Link
                          ...
                          className={`dash-chip text-xs font-semibold transition-colors ${
                            filters.contentLanguage === row.language
                              ? "bg-brand text-white shadow-[inset_0_0_0_1px_var(--ptec-brand)]"
                              : "dash-status--neutral hover:bg-brand/10 hover:text-brand"
                          }`}
                        >
        ...
        {totalPages > 1 && (
          <nav aria-label={t("paginationLabel")} className="flex items-center justify-between gap-2 border-t border-divider/70 px-4 py-2.5">
            ...
            <div className="flex gap-1.5">
              {data.page > 1 && (
                <Link href={hrefFor(preset, data.page - 1)} className="rounded-lg border border-divider px-2.5 py-1 text-xs font-semibold text-text-body hover:border-brand/40 hover:text-brand focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand">
                  {t("prevPage")}
                </Link>
              )}
      ...
      <section aria-labelledby="dept-heading" className="dash-card overflow-hidden">
        <div className="flex items-center gap-2.5 px-4 pt-3.5">
          ...
          <div>
            <h3 id="dept-heading" className="text-sm font-bold text-text-heading">
              {t("deptTitle")}
            </h3>
            <p className="text-xs text-text-muted">{t("deptFormula")}</p>
          </div>
        </div>
              ...
              <tbody>
                {data.departments.map((d) => {
                  ...
                  <tr key={d.name} className="dash-row border-b border-divider/60 last:border-b-0">
                    <th scope="row" className="max-w-[220px] dash-truncate-head px-4 py-2 text-start font-semibold text-text-heading">
                      {d.name}
                    </th>
