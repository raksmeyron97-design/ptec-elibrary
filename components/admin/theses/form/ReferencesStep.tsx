        ...
        {references.map((ref, i) => (
          <div key={i} className="flex items-start gap-2 group">
            <div className="mt-1.5 flex flex-col opacity-0 group-hover:opacity-100 transition-opacity">
              <button type="button" disabled={disabled || i === 0} onClick={() => moveRef(i, -1)} aria-label="Move reference up" className="text-text-muted/50 hover:text-brand disabled:opacity-20">
                <ChevronUp className="w-3.5 h-3.5" />
              </button>
              <button type="button" disabled={disabled || i === references.length - 1} onClick={() => moveRef(i, 1)} aria-label="Move reference down" className="text-text-muted/50 hover:text-brand disabled:opacity-20">
            ...
            <div className="flex-1">
              ...
              {URL_OR_DOI_RE.test(ref) && (
                <span className="mt-1 inline-flex items-center gap-1 text-[11px] text-emerald-600">
                  <Link2 className="h-3 w-3" /> Link/DOI detected
                </span>
              )}
            </div>
            <button type="button" onClick={() => removeRef(i)} disabled={disabled} aria-label="Remove reference" className="mt-2 text-text-muted/50 hover:text-red-500 transition-colors disabled:opacity-40">
      ...
      {showPasteArea ? (
        <div className="p-3 bg-paper border border-divider rounded-lg space-y-2">
          <textarea
            ...
            disabled={disabled}
            rows={4}
            placeholder="Paste your block of references here..."
