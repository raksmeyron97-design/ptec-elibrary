...
// Rating + comment form for publications — adapted from
// components/ui/reviews/ReviewForm.tsx (books), posting to
// submitPublicationReview instead.

import { useState, useTransition, useRef } from "react";
import { submitPublicationReview } from "@/app/actions/publication-reviews";

type PublicationReviewFormProps = {
...
export default function PublicationReviewForm({
  publicationId,
  publicationSlug,
  existingRating,
  existingContent,
}: PublicationReviewFormProps) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await submitPublicationReview(publicationId, publicationSlug, formData);
      setResult(res);
  ...
  return (
    <div className="rounded-xl border border-divider bg-bg-surface p-6 shadow-sm">
      <h3 className="mb-1 text-base font-bold text-text-heading">
        {isEdit ? "Update your review" : "Rate this publication"}
      </h3>
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label
            htmlFor="publication-review-content"
          ...
          <textarea
            id="publication-review-content"
            name="content"
            rows={4}
            defaultValue={existingContent ?? ""}
            placeholder="What did you find most useful about this publication?"
