  ...
  return (
    <div className="rounded-2xl border border-divider bg-bg-surface p-4 shadow-sm">
      <p className="mb-3 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-text-muted">
        <GraduationCap className="h-3.5 w-3.5" /> Continue Your Learning Path
      </p>
        ...
        {paths.map((p) => {
          ...
          return (
            <Link
              key={p.id}
              href={`/paths/${p.slug}`}
              className="group flex items-center gap-3 rounded-xl border border-divider/60 bg-paper/40 p-2.5 transition hover:border-brand/30 hover:bg-paper"
            >
              {p.cover_url ? (
                ...
                <div className="flex h-12 w-9 shrink-0 items-center justify-center rounded-md border border-divider/60 bg-brand/5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex items-center gap-2">
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-divider/60">
                    <div className="h-full rounded-full bg-brand transition-all" style={{ width: `${pct}%` }} />
                  </div>
                  <span className="shrink-0 text-[10.5px] font-semibold text-text-muted">{pct}%</span>
                </div>
              </div>
              <ChevronRight className="h-4 w-4 shrink-0 text-text-muted/50 transition-transform group-hover:translate-x-0.5 group-hover:text-brand" />
            </Link>
          );
        })}
      </div>
