...
export default function FilterSidebar({
  ...
  return (
    <div className="sticky top-[88px] rounded-[20px] border border-divider bg-bg-surface p-[22px]">
      {/* Header */}
      <div className="mb-5 flex items-center justify-between">
        <h2 className="text-base font-bold text-text-heading">Filters</h2>
            ...
            <Link
              ...
              className={`flex items-center justify-between rounded-[11px] px-3 py-[9px] text-sm transition-all duration-150 ${
                active
                  ...
                  : "font-medium text-text-body hover:bg-paper hover:text-text-heading"
              }`}
            >
              <span>{dept}</span>
              {active && (
              ...
              <Link
                ...
                className={`rounded-[11px] border px-3 py-2.5 text-center text-[13px] font-semibold transition-all duration-150 ${
                  active
                    ...
                    : "border-divider text-text-body hover:border-brand/50 hover:text-brand"
                }`}
              >
                {fmt}
              </Link>
...
function FilterSection({
  ...
  return (
    <div className={last ? "" : "mb-6"}>
      <h3 className="mb-3 text-[11px] font-bold uppercase tracking-[0.1em] text-text-muted">
        {title}
      </h3>
