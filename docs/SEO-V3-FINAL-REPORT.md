...
**The single most valuable finding is not a code change.** SEO V2 is committed
but **never merged or deployed** — `/subjects` and `/authors` return 404 in
production, and ten empty subject pages are still in the live sitemap. Merging
it is worth more than everything below.
...
**Not run:** the Playwright e2e suite. It needs the CI `e2e` job (local Supabase
+ migrations + seed). This remains the same verification gap SEO V2 recorded.
...
- **Merge and deploy SEO V2 + V3.** Highest-value item in this report.
...
- Nothing here has been deployed; every "verified live" line describes
  production **as it is today**, running pre-V2 code.
...
- The e2e suite was not executed.
