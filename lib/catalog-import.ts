...
export const FIELD_MAX: Record<string, number> = {
  ...
  ddc: 80, shelf_location: 60, accession_number: 60, barcode: 32, description: 5000,
};
...
export const IMPORT_FIELDS = [
  ...
  "department", "ddc", "shelf_location", "copies_total", "description",
...
export const FIELD_DOCS: FieldDoc[] = [
  ...
  { field: "ddc", required: false, type: "text ≤ 80", example: "372.7 BIL", rule: "Dewey Decimal class — what the book is ABOUT. Any format: 372.7, 372.7 BIL, or a Khmer local code (ស.គ). Use shelf_location for the physical shelf." },
  { field: "shelf_location", required: false, type: "text ≤ 60", example: "A-1-01", rule: "Free text (e.g. shelf / rack code) — where the copy physically SITS. Not the Dewey class (see ddc)." },
...
export const CSV_TEMPLATE_EXAMPLE =
  `${CSV_TEMPLATE_HEADER}
Introduction to Law,John Smith,978-0-306-40615-7,PTEC Press,2020,en,Law,Public Law,372.7 BIL,A-1-01,1,"A comprehensive introduction to law, with case studies.",ACC-001,33697,https://drive.google.com/file/d/1QuNSZO4OMf2tTlv89GfG4PGdCK2VE2sW/view?usp=sharing,"law, intro, guide"
Introduction to Law,John Smith,978-0-306-40615-7,PTEC Press,2020,en,Law,Public Law,372.7 BIL,A-1-01,1,"A comprehensive introduction to law, with case studies.",ACC-002,33698,https://drive.google.com/file/d/1QuNSZO4OMf2tTlv89GfG4PGdCK2VE2sW/view?usp=sharing,"law, intro, guide"
ច្បាប់រដ្ឋប្បវេណី,ក សុខា,,,2019,km,Law,Civil Law,ស.គ,B-2-05,1,ច្បាប់រដ្ឋប្បវេណីខ្មែរ,ACC-003,33699,,ច្បាប់
`;
...
const HEADER_ALIASES: Record<BookImportField, string[]> = {
  ...
  // `call_number` maps to the BOOK's ddc here on purpose: the wizard has no
  // copy-level call-number field (catalog_copies.call_number is generated by
  // generateCopies(), never imported), so there is nothing to collide with, and
  // a PMB-style "call number" column IS the Dewey class.
  ddc: ["ddc", "dewey", "dewey_decimal", "ddc_number", "classification", "class_number", "call_no", "callnumber", "call_number", "លេខរៀបចំ", "ddc_code"],
...
export function validateRow(
  ...
  if (!ddcRes.ok) err("FIELD_TOO_LONG", ddcRes.error, "ddc");
