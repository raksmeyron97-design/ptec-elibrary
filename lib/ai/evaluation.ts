...
/** Share of a slug's content words that must appear for the question to NAME that work. */
const NAMED_WORK_COVERAGE = 0.6;

/**
 * Does the question NAME this work, rather than merely being labelled against
 * it?
 *
 * This is the discriminator, and the category is not. Three questions the
 * v2 fixture files under comparison/synthesis show why:
 *
 *   "Compare 'Research Methods in Education (5th Edition)' and '… (8th)'"
 *        names both works verbatim → multi_document, both required
 *   "What is the difference between validity and reliability?"
 *        labelled against EIGHT books, naming none → topic_unscoped
 *   "Across the library's books, how is mixed methods handled?"
 *        labelled against six, naming none → topic_unscoped
 *
 * Counting sources cannot separate these — the phonics question is labelled
 * against four, under the recall-list threshold, and still names nothing.
 * Reading the question can, and it is a pure function of two strings.
 */
export function namesWork(question: string, slug: string): boolean {
  const text = question
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
  ...
  return present >= Math.max(2, Math.ceil(words.length * NAMED_WORK_COVERAGE));
}
...
export interface QuestionLabel {
  id: string;
  category: string;
  /** The question as asked. Read to decide whether it NAMES the works it is labelled against. */
  question?: string;
...
export function deriveEvidenceScope(q: QuestionLabel): EvidenceScope {
  ...
  // 5. The question NAMES two or more works, so all of them are required —
  //    decided by reading the question, never by its category (`namesWork`).
  const named = q.question ? sources.filter((slug) => namesWork(q.question!, slug)) : [];
  if (named.length >= 2) return "multi_document";

  // 6. ONE labelled record is a choice somebody made: the label is exhaustive
  //    and precision against it means something.
  if (sources.length === 1) return "single_document";

  // 7. Several records, none of them named in the question. However many there
  //    are, that label was produced by scanning the corpus rather than chosen,
  //    and good evidence outside it is still good evidence. Counting is what
  //    got this wrong before: v2's phonics question is labelled against four,
  //    under any plausible "recall list" threshold, and names nothing.
 ...
 *
 * The NAMED works, not every labelled one: "compare A and B" may be labelled
 * against a third book that also discusses the topic, and requiring that one
 * too would fail a correct comparison.
 */
export function requiredDocuments(q: QuestionLabel, scope: EvidenceScope): string[] {
  if (scope !== "multi_document") return [];
  if (q.requiredDocuments) return q.requiredDocuments;
  const sources = q.sources ?? [];
  const named = q.question ? sources.filter((slug) => namesWork(q.question!, slug)) : [];
  return named.length >= 2 ? named : sources;
}
