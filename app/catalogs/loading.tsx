          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2 overflow-hidden">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="h-8 shrink-0 rounded-full bg-slate-100 animate-pulse"
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:block h-7 w-16 rounded-full bg-slate-100 animate-pulse" />
              <div className="hidden sm:block h-7 w-12 rounded-full bg-slate-100 animate-pulse" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-6 md:px-12">
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-9 w-9 rounded-lg bg-slate-100 animate-pulse" />
          ))}
        </div>
      </div>
    </div>
