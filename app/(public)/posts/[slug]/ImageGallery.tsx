...
/* ─── Lightbox top-bar: Download + Share + Counter ─── */
function LightboxTopBar({ imageUrl, pageUrl, postTitle, index, total }: {
  imageUrl: string; pageUrl: string; postTitle: string; index: number; total: number;
}) {
  const [shareOpen, setShareOpen] = useState(false);
  ...
  return (
    <div
      className="absolute top-4 left-1/2 -translate-x-1/2
                 flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-3 py-2 z-10"
      onClick={(e) => e.stopPropagation()}
    >
      {/* Download */}
      <a
...
export default function ImageGallery({
  ...
  return (
    <>
      ...
      {/* ── Photo grid ── */}
      <div className="grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-4" style={{ gridAutoRows: "1fr" }}>
        {urls.map((url, i) => (
          <Tile
            key={url + i}
            url={url}
      ...
      {lightboxIdx !== null && (
        ...
        >
          ...
          {/* Top bar: download + share + counter */}
          <LightboxTopBar
            imageUrl={urls[lightboxIdx]}
            pageUrl={pageUrl}
            postTitle={postTitle}
            index={lightboxIdx}
            total={urls.length}
