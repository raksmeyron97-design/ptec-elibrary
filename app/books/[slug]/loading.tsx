          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="h-6 w-20 animate-pulse rounded-full bg-slate-200" />
              ))}
            </div>
            {/* Title */}
            ...
            <div className="flex gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="h-5 w-5 animate-pulse rounded bg-slate-200" />
              ))}
            </div>
            {/* Summary lines */}
            ...
            <div className="grid gap-3 pt-2 sm:grid-cols-2">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="h-[60px] animate-pulse rounded-[13px] bg-slate-100" />
              ))}
            </div>
            {/* Action buttons */}
            <div className="flex gap-3 pt-2">
              <div className="h-[50px] w-40 animate-pulse rounded-[14px] bg-slate-200" />
              <div className="h-[50px] w-28 animate-pulse rounded-[14px] bg-slate-100" />
            </div>
          </div>
        </div>
