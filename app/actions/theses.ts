...
/** Blocks the transition to published/scheduled when required fields (spec §26)
 *  are missing. Shared with the review queue via lib/publish-readiness.ts. */
function checkPublishReady(merged: Partial<ThesisData>): string | null {
  return checkThesisPublishReady(merged as Record<string, unknown>);
}
...
async function uniqueThesisSlug(supabase: ReturnType<typeof createServiceClient>, base: string, ignoreId?: string): Promise<string> {
  const safeBase = (base && base.trim()) || "thesis";
  let slug = safeBase;
  let n = 1;
  while (true) {
...
export async function deleteThesis(id: string) {
  ...
  // Full-text page index, chunk embeddings and the index-state row
    // (no FK — polymorphic record_id, migrations 0066/0082/0133)
