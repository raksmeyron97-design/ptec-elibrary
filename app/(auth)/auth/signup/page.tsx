...
function friendlyError(msg: string): string {
  ...
  if (/signup rejected|admin/i.test(msg))
    return "Signup rejected. This email domain may be restricted. Please contact the administrator.";
  return msg;
}
...
function getStrength(pw: string): { level: 0 | 1 | 2 | 3; label: string } {
  ...
  if (pw.length < 16) return { level: 3, label: "Good" };
  return { level: 3, label: "Strong" };
}

const STRENGTH_COLORS = ["bg-paper", "bg-red-400", "bg-amber-400", "bg-emerald-400"] as const;
...
export default function SignupPage() {
  ...
  const nameErrMsg = submitted && fullName.trim() === "" ? "Full name is required." : null;
  const emailErrMsg =
    submitted && email === ""
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (nameErrMsg || emailErrMsg || passwordErrMsg || confirmErrMsg) return;
    ...
    if (!verification.success) {
      setError(friendlyError(verification.error || "Signup rejected."));
      setLoading(false);
      return;
    }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-paper px-6">
        <div
          role="status"
          aria-live="polite"
          className="w-full max-w-md rounded-2xl border border-divider bg-bg-surface p-10 shadow-lg text-center"
        >
          ...
          <Link
            href="/auth/login"
            className="mt-8 inline-flex h-11 items-center justify-center rounded-xl bg-blue-950 px-6 text-sm font-semibold text-white transition hover:bg-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40"
          >
            Back to Login
          </Link>
        </div>
    ...
    <div className="flex min-h-screen font-sans">
      ...
      <div className="relative hidden w-[45%] flex-col justify-between overflow-hidden bg-blue-950 p-12 lg:flex">
        ...
        <Link href="/" className="relative flex items-center gap-4 group w-fit">
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-bg-surface/10 border border-white/10 transition group-hover:bg-bg-surface/20">
        ...
        <div className="relative space-y-3">
          {[
            ...
            <div
              key={text as string}
              className="flex items-center gap-4 rounded-xl border border-white/10 bg-bg-surface/5 px-4 py-3.5 backdrop-blur-sm"
            >
          ...
          <form onSubmit={handleSignup} noValidate className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                aria-describedby={nameErrMsg ? "signup-name-error" : undefined}
              ...
              />
              {nameErrMsg && (
                <p id="signup-name-error" role="alert" className="mt-1.5 text-xs text-red-500">
                  {nameErrMsg}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {emailErrMsg && (
                <p id="signup-email-error" role="alert" className="mt-1.5 text-xs text-red-500">
                  {emailErrMsg}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {passwordErrMsg && (
                <p id="signup-password-error" role="alert" className="mt-1.5 text-xs text-red-500">
                  {passwordErrMsg}
                </p>
              )}
              {/* Strength meter */}
              {password && !passwordErrMsg && (
                ...
                >
                  {([1, 2, 3] as const).map((tier) => (
                    <div
                      key={tier}
                      className={`h-1 flex-1 rounded-full transition-colors motion-safe:transition-colors ${
                        strength.level >= tier ? STRENGTH_COLORS[tier] : "bg-divider"
                      }`}
                    />
                  ))}
                  <span className="ml-2 text-[11px] text-text-muted">{strength.label}</span>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              />
              {confirmErrMsg && (
                <p id="signup-confirm-error" role="alert" className="mt-1.5 text-xs text-red-500">
                  {confirmErrMsg}
                </p>
              )}
              {!confirmErrMsg && confirm && confirm === password && (
                <p id="signup-confirm-ok" className="mt-1.5 text-xs text-emerald-600">
                  Passwords match
                </p>
              )}
            </div>
