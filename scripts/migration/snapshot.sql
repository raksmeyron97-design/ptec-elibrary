...
select c.relname||' rls='||c.relrowsecurity from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind='r' order by 1;
\echo === row counts
...
select conrelid::regclass::text||' '||conname||' '||contype||' '||pg_get_constraintdef(oid) from pg_constraint where connamespace='public'::regnamespace order by 1;
\echo === indexes (public)
...
select p.proname||'('||pg_get_function_identity_arguments(p.oid)||') secdef='||p.prosecdef||' lang='||l.lanname||' md5='||md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace join pg_language l on l.oid=p.prolang where n.nspname='public' order by 1;
\echo === function grants (public)
select p.proname||' '||coalesce(array_to_string(p.proacl,','),'(default)') from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' order by 1, 2;
\echo === triggers
select t.tgrelid::regclass::text||' '||t.tgname||' '||pg_get_triggerdef(t.oid) from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace where not t.tgisinternal and (n.nspname='public' or t.tgrelid='auth.users'::regclass) order by 1, 2;
\echo === views (public)
...
select tablename||' '||policyname||' '||cmd||' roles='||array_to_string(roles,',')||' using='||coalesce(md5(qual),'-')||' check='||coalesce(md5(with_check),'-') from pg_policies where schemaname='public' order by 1,2;
\echo === table grants (public)
select table_name||' '||grantee||' '||string_agg(privilege_type, ',' order by privilege_type) from information_schema.role_table_grants where table_schema='public' and grantee in ('anon','authenticated','service_role') group by 1,2 order by 1,2;
