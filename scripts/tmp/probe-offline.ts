...
(async () => {
  const pdf = makeLargeTestPdf({ pages: 300, bytesPerPage: 64 * 1024, label: "outage" });
  ...
  await page.getByRole("dialog", { name: /go to page/i }).getByRole("textbox").fill("80");
  ...
  await state("offline on 160");
  await ctx.setOffline(false);
  for (const t of [3000, 5000, 10000]) { await wait(t); await state(`recovering +${t}`); }
